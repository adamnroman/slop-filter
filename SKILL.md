...
1. Clone the repo to `~/slop-filter`. If the folder exists, run `git pull` in it instead.
   - Pick the folder once. Chrome ties an unpacked extension to its folder path. Moving or renaming the folder later makes Chrome treat it as a new extension and drop the saved API key, settings, and labels.
...
- After any change: bump `version` in `manifest.json`, run `node --test`, then have the user click "Reload extension" and reload their tabs. You cannot reload it for them.
