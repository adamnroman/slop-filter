...
- **YouTube** videos, on the home grid and in search results (`ytd-rich-item-renderer`, `ytd-video-renderer`): a video is judged by its script. `loadText` gets the caption track by asking YouTube's player endpoint as the Android app, downloads the captions, and `src/transcript.js` picks a 150 word window from the first minute, at a position seeded by the video id so a video's verdict is stable. The excerpt goes to Jev with `kind: 'transcript'`: the questions about typing and punctuation are left out, the rest apply, and auto-captions carry no punctuation so the punctuation features read as 0. Requests go out without cookies. English captions only. Each video costs two requests to YouTube, about half a megabyte, at most three at a time. Videos with no captions, Shorts, playlists, and ads are left alone with no chip and no error. The "Score YouTube videos" switch on the options page turns video cards off entirely: the adapter's `extract` returns null for them, so nothing is fetched, and comments still work. Flipping it re-decides every element on the page. The route is fragile: the website's own player call, its caption links, and the transcript panel's endpoint all refuse a page script as of 2026-09, and YouTube changes this often. If videos stop getting scores, start in `src/sites/youtube.js`.
- **YouTube** comments: every comment is a `ytd-comment-view-model` with its text in `#content-text`, complete even behind "Read more". The id is the `lc` parameter of the timestamp link. A reply's parent is the top comment of its thread, and a top comment's parent is the video title. The chip goes inline at the end of the header row `#header-author`. YouTube enforces Trusted Types, so never use `innerHTML`. Judging the videos themselves by their scripts is parked on the `feat/youtube` branch.
...
Firefox's MV3 manifest uses `background.scripts` with module support. The source stays shared and there is only one `manifest.json`: `scripts/manifests.mjs` derives the Firefox manifest at packaging time, changing only `background.service_worker` to `background.scripts` and adding `browser_specific_settings` with the project's add-on id. Everything else, permissions and content scripts included, is inherited from the shared manifest.
...
- **Word formation**: tells in how words are made, not in sentence shape. A compressed coinage packs an idea into a noun stack and uses it as a known term ("keep rate", "review-rejection rate"); a person says the idea. Frame presupposition refers to a set the reader never saw ("the missing one is..."). The spotlight formula singles out one detail with casual praise ("is the part that jumps out"). Compressed coinages and the spotlight formula are hard rules.
...
- Browser packages: `node scripts/zip.mjs chrome` builds `dist/slop-filter-<version>-chrome.zip`; `node scripts/zip.mjs firefox` builds `dist/slop-filter-<version>-firefox.zip`. Each archive contains `manifest.json` at its root. Upload the Chrome archive to the Chrome Web Store and the Firefox archive to AMO. The Firefox manifest is derived from the shared one (see "Install > Firefox"), so there is no second manifest to keep in sync. Firefox validation is optional and manual: extract an archive and run `npx web-ext@8.9.0 lint --source-dir <extracted package>`, then load it in `about:debugging`. Nothing in the repo installs or runs `web-ext`.
